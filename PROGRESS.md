...
**最后更新**：2026-04-29
**状态**：全站已部署到 GitHub Pages，Giscus 评论已启用，4 轮视觉/性能优化完成，第 5 轮"迁移收尾"完成（菜单图标统一、邮箱可见、写在前面整合 AI 散文、10 项美学打磨），README/CLAUDE.md/PROGRESS.md 三件套齐全（149 页，主页 0 外部阻塞资源）
...
- ~~**一些资料页**：原站有密码保护的电子书下载链接未迁移~~ → 第 5 轮已从主菜单删除该入口（页面文件 `content/page/resources/` 也已删除）
- **电子书集合 / 向我提问 / Contact 等次要页**：内容已迁移但未放入主菜单，需要时可加

---

## ✅ 已完成（第五轮：迁移收尾 + 美学打磨，2026-04-29）

为整个迁移工作画一个收尾：内容核对、菜单清理、首页视觉一致性、文章页排版打磨。

### 1. 原站内容覆盖二次核对
- 用 `crawl/listing/posts_p1.json`（48 篇 WP 文章）逐条比对本地 `content/post/` 和 `content/page/`，slug 经过中文标点剥离后 100% 命中（如 `《乌合之众》读书笔记` → `乌合之众读书笔记`）
- 确认 13 个 WP page 全部就位（含被升级为顶部菜单的 `关于作者` / `写在前面`，以及被英文化的 `evaluation` / `course-notes` / `thoughts` 等聚合页）

### 2. 菜单清理：删除"一些资料"
- `hugo.yaml` 移除 `identifier: resources` 整段菜单项
- `content/page/resources/` 目录整体删除（页面文件也不再保留）

### 3. "写在前面"改写：纳入 AI 时代散文
- `content/page/preface/index.md` 用《在 AI 时代留下一点自己的痕迹》作为正文主体（同一篇散文也作为博客文章发表在 `/2026/04/26/在ai时代留下一点自己的痕迹/`）
- 末尾保留 2021 年初版"写在前面"作为 `### 附：2021 年版` 引用块，串起新旧两版自我

### 4. 左侧菜单图标统一
- 原本只有 `主页` / `关于作者` / `随感录` 有图标，其余四项裸文字不一致
- 在 `assets/icons/` 新增 4 个 Tabler 风格 SVG（24×24，stroke-width=2，stroke=currentColor）：`book-open` / `school` / `notebook` / `code`
- `hugo.yaml` 给 `preface` / `course-review` / `course-notes` / `notes` 菜单项补齐 `params.icon`
- 最终 7 个主菜单项全部带图标

### 5. 邮箱"可见 + 可点"
- 主题原版只在头像下放图标，鼠标悬停才显示邮箱，导致很多人以为不可点
- 新增 `layouts/_partials/sidebar/left.html`（覆盖主题），在 `.menu-social` 之后插入 `.sidebar-contact` 块：
  - 邮箱：`<a href="mailto:...">` 显式可见 + 仍是超链接
  - 微信号：静态 `<span>`，可见、可选中复制
- `hugo.yaml` 在 `params.sidebar.contact` 下声明 `email` / `wechat` 字段
- `custom.scss` 新增 `.sidebar-contact` 样式（小字号 1.2rem，长邮箱允许换行，hover 变蓝；compact 与窄屏居中）

### 6. 美学打磨 10 项（`assets/scss/custom.scss` 末段）
| # | 主题 | 实现 |
|---|---|---|
| ① | 引用块 | 左色条 + 蓝→透明渐变背景 + 斜体；嵌套引用色更深；深色模式适配 |
| ② | h2 章节标记 | 标题左侧 3px 渐变色细条（蓝→深蓝） |
| ③ | `<hr>` 分隔线 | 替换为 `✦  ✦  ✦` 三连点花饰，letter-spacing 0.6em |
| ④ | sidebar-contact 顶部 | 一道 60% 宽度的渐隐细线，与主页底部署名同语言 |
| ⑤ | 文本选中 | 蓝色 22% 透明 + 深色模式 32% 透明 |
| ⑥ | 链接 hover | 微弱蓝色底纹（rgba 0.06）+ 圆角 + 内边距过渡 |
| ⑦ | 文章卡片 meta | 时间/描述字号降到 1.3rem + 三级文字色，不抢标题戏 |
| ⑧ | 主页 4 张卡片 emoji | 字体兜底栈（Apple/Segoe/Noto Color Emoji）解决 Windows 渲染偏扁 |
| ⑨ | 段落留白 | `> p { margin: 1.1rem 0; line-height: 1.85 }` 更克制 |
| ⑩ | 文章首段"序章感" | `.main-article .article-content > p:first-of-type` 字号 1.7rem，三级文字色，缓缓进入正文 |

### 踩过的坑
- 一开始把 ⑩ 写成 `.article-page`，但 Stack 主题文章 wrapper 实际是 `<article class="main-article">`，已修正
- 一开始把 ⑦ 写成 `.article-time` / `.article-details time`，但 compact 列表的实际结构是 `<footer class="article-meta"><time>`，已修正为 `.article-list article .article-meta` + 子元素
- `.sidebar.compact .sidebar-contact, @media (max-width: 1024px) { ... }` 是非法 CSS（不能把类选择器和 `@media` 用逗号并列），已拆成两段独立规则
- Python 迁移核对脚本在 Windows 控制台用了 `❌` 触发 `gbk` 编码错误，改用 `[MISS]` 并 `sys.stdout.reconfigure(encoding='utf-8')`
